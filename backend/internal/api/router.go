...
func NewRouter(store *storage.Store) *gin.Engine {
	...
	// 自定义静态文件处理，支持中文路径 URL 编码
	router.GET("/data/*filepath", handler.serveStaticFile)

	api := router.Group("/api/v1")
	{
		api.GET("/audio", handler.listAudio)
...
// serveStaticFile 处理静态文件访问
func (h *Handler) serveStaticFile(c *gin.Context) {
	filepathParam := c.Param("filepath")
	// 构建完整文件路径
	fullPath := "." + filepathParam
...
func (h *Handler) listAudio(c *gin.Context) {
	...
	for _, file := range files {
		...
		audioList = append(audioList, audioItem{
			ID:       len(audioList) + 1,
			Filename: name,
			Name:     displayName,
			URL:      "/data/" + name,
